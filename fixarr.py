        ...
        for name in files:
            ...
            if not name.endswith(tuple(ext)):
                ...
                if data["results"]:
                    ...
                    # with DDGS() as ddgs:
                    #     ddgs_gen = ddgs.text(search, backend="lite")
                 ...
                 temperature=0)   
                    ...
                    template = f"your task is grab movie name and year using following format Name: Value Goes To here else set as None Year: Value Goes To here else set as None get data from {base_name} dont use illegal characters = (,),$,/,\,*,?,<,>,|,.,$,:)",
                    ...
                    outs = chain.run(input=template)
                        
                    split_word = outs.split(" ")
                    
                    rich.print(split_word)
                    
                    names = split_word[0].replace('"', '').replace("''","").replace(":","").strip()
                    
                    year = split_word[1].replace('"', '').replace("''","").replace(":","").strip()
                    ...
                    #Construct the new file name with the extracted movie title, release year, and original file extension
                    new_name = (
                        f"{names} ({year}){ext}" if year else f"{names}{ext}"
                    )
                    ...
                    # create files for folders and rename
                    folder_name = f"{names} ({year})" if year else f"{names}"
                    folder_path = os.path.join(file_or_folder, folder_name)
